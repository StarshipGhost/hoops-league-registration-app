    ...
    <ul className="hidden lg:flex items-center gap-4">
      {links.map((link, index) => 
          <HeaderLink 
              ...
              link={link} 
              style={desktop} 
              scrollFunction={() => scrollFunction(index + 1)} />
      )}
    ...
    <ul className={`w-full bg-white dark:bg-black flex flex-col gap-2 p-4 absolute z-1 transition duration-200 ease-linear delay-100 -translate-y-full border-box lg:hidden ${open && `translate-y-0`}`} >
      {links.map((link, index) => 
          <HeaderLink 
              ...
              link={link} 
              style={mobile} 
              scrollFunction={() => navigate(index + 1)} />
            )}
      <li><OrangeButton extra="w-full px-4 py-2" text="Join Now" onClick={() => navigate(5)}/></li>
    </ul>
  )
}
...
const Header = () => {
  ...
  const header : RefObject<HTMLElement | null> = useRef<HTMLElement | null>(null)
